  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-2rem)] max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-mist bg-panel shadow-dialog data-[state=open]:animate-scale-in">
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-mist bg-panel px-6 py-4">
            <Dialog.Title className="text-base font-semibold text-ink">
              {isEditing ? "Edit Category" : "Create Category"}
            </Dialog.Title>
            <Dialog.Close asChild>
              <button className="flex h-8 w-8 items-center justify-center rounded-lg text-text-muted hover:text-ink hover:bg-accent-soft transition" aria-label="Close">
                <X className="h-4 w-4" />
              </button>
            </Dialog.Close>
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-[0.3em] text-text-muted font-medium">Category Image</label>
              <div className="flex items-start gap-4">
                <div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-xl border border-mist bg-canvas overflow-hidden">
                  {form.image ? (
                ...
                <div className="flex-1 space-y-2">
                  ...
                  />
                  <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-mist px-4 py-2 text-xs text-text-muted hover:text-ink hover:border-ink/30 transition">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="relative">
                ...
                <Hash className="absolute right-3 top-1/2 mt-3 h-4 w-4 text-text-muted/40" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs uppercase tracking-[0.3em] text-text-muted font-medium">Order</label>
                <input type="number" value={form.order} onChange={(e) => updateField("order", parseInt(e.target.value) || 0)}
                  className="w-full rounded-xl border border-mist bg-canvas px-4 py-3 text-sm text-ink outline-none focus:border-ink/30 focus:ring-1 focus:ring-ink/10" min="0" />
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1.5 text-xs uppercase tracking-[0.3em] text-text-muted font-medium">
                ...
                <textarea value={form.descriptionEn} onChange={(e) => updateField("descriptionEn", e.target.value)}
                  className="min-h-[80px] rounded-xl border border-mist bg-canvas p-3 text-sm text-ink outline-none resize-y focus:border-ink/30 focus:ring-1 focus:ring-ink/10" />
              </label>
              <label className="flex flex-col gap-1.5 text-xs uppercase tracking-[0.3em] text-text-muted font-medium">
                Description (Bengali)
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <label className="flex flex-col gap-1.5 text-xs uppercase tracking-[0.3em] text-text-muted font-medium">
                ...
                <select value={form.parentId} onChange={(e) => updateField("parentId", e.target.value)}
                  className="rounded-xl border border-mist bg-canvas px-4 py-3 text-sm text-ink outline-none focus:border-ink/30 focus:ring-1 focus:ring-ink/10">
              ...
              <div className="flex items-end pb-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-xs font-medium text-ink">Featured Category</span>
                </label>
              </div>
            </div>
            ...
            <div>
              <button onClick={() => setSeoOpen(!seoOpen)}
                className="flex items-center gap-2 text-xs uppercase tracking-[0.3em] text-text-muted font-medium hover:text-ink transition">
                <Globe className="h-3.5 w-3.5" />
                SEO Settings
                <span className="ml-1 text-text-muted/50">{seoOpen ? "▲" : "▼"}</span>
              </button>
              {seoOpen && (
                <div className="mt-4 space-y-4 border border-mist rounded-xl p-4 bg-canvas/50">
                  <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className="flex flex-col gap-1.5 text-xs uppercase tracking-[0.3em] text-text-muted font-medium">
                      ...
                      <textarea value={form.metaDescriptionEn} onChange={(e) => updateField("metaDescriptionEn", e.target.value)}
                        className="min-h-[60px] rounded-xl border border-mist bg-canvas p-3 text-sm text-ink outline-none resize-y focus:border-ink/30 focus:ring-1 focus:ring-ink/10"
                        placeholder="Brief description for search engines" maxLength={160} />
                      <span className="text-[10px] text-text-muted/60 text-right">{form.metaDescriptionEn.length}/160</span>
                    </label>
                    <label className="flex flex-col gap-1.5 text-xs uppercase tracking-[0.3em] text-text-muted font-medium">
                      Meta Description (Bengali)
                      <textarea value={form.metaDescriptionBn} onChange={(e) => updateField("metaDescriptionBn", e.target.value)}
                        className="min-h-[60px] rounded-xl border border-mist bg-canvas p-3 text-sm text-ink outline-none resize-y focus:border-ink/30 focus:ring-1 focus:ring-ink/10" maxLength={160} />
                      <span className="text-[10px] text-text-muted/60 text-right">{form.metaDescriptionBn.length}/160</span>
                    </label>
                  </div>
                  <div className="rounded-lg border border-mist/50 bg-canvas p-3">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-text-muted font-medium mb-1">Preview</p>
          ...
          <div className="sticky bottom-0 flex items-center justify-end gap-2 border-t border-mist bg-panel px-6 py-4">
            <Dialog.Close asChild>
